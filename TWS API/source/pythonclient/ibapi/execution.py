    ...
    def __str__(self):
        return "ExecId: %s, Time: %s, Account: %s, Exchange: %s, Side: %s, Shares: %f, Price: %f, PermId: %d, " \
               ...
               "EvMultiplier: %f, ModelCode: %s, LastLiquidity: %d" % (self.execId, self.time, self.acctNumber,
                                                                       self.exchange, self.side, self.shares,
                                                                       self.price, self.permId, self.clientId,
                                                                       self.orderId, self.liquidation,
                                                                       self.cumQty, self.avgPrice, self.orderRef,
                                                                       self.evRule, self.evMultiplier, self.modelCode,
                                                                       self.lastLiquidity)
