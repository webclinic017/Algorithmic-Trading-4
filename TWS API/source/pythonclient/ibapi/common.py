...
class RealTimeBar(Object):
    ...
    def __str__(self):
        return "Time: %d, Open: %f, High: %f, Low: %f, Close: %f, Volume: %d, WAP: %f, Count: %d" % (
            self.time, self.open_, self.high,
...
class DepthMktDataDescription(Object):
    ...
    def __str__(self):
        if (self.aggGroup != UNSET_INTEGER):
            ...
            aggGroup = ""
        return "Exchange: %s, SecType: %s, ListingExchange: %s, ServiceDataType: %s, AggGroup: %s, " % (
            self.exchange, self.secType, self.listingExch, self.serviceDataType, aggGroup)
...
class HistoricalTickBidAsk(Object):
    ...
    def __str__(self):
        return "Time: %d, TickAttriBidAsk: %s, PriceBid: %f, PriceAsk: %f, SizeBid: %d, SizeAsk: %d" % (
            self.time, self.tickAttribBidAsk, self.priceBid, self.priceAsk, self.sizeBid, self.sizeAsk)
...
class HistoricalTickLast(Object):
    ...
    def __str__(self):
        return "Time: %d, TickAttribLast: %s, Price: %f, Size: %d, Exchange: %s, SpecialConditions: %s" % (
            self.time, self.tickAttribLast, self.price, self.size, self.exchange, self.specialConditions)
